	...
	if (SpellAlertSCTDB[playerName].Colorize) then
		if (SpellAlertSCTDB[playerName].Compact) then
			if (msgtype == "cast") then
				msgtodisplay = "|cff68ccef"..mob.."|cffffffff > |cffff0000"..spell;
			...
			if (GetLocale()=="zhCN") then
				if (msgtype == "cast") then
					msgtodisplay = "|cff68ccef"..mob.."|cffffffff ".. SASCT_BEGIN_CAST .." ".."|cffff0000"..spell; 
				elseif (msgtype == "gain") then
					msgtodisplay = "|cff68ccef"..mob.."|cffffffff ".. SASCT_GAINS .." ".."|cffff0000"..spell;
				end
			end
		end
	else
		if (SpellAlertSCTDB[playerName].Compact) then
			...
			if (GetLocale()=="zhCN") then
				if (msgtype == "cast") then
					msgtodisplay = mob..SASCT_EMOTESPACE.. SASCT_BEGIN_CAST ..SASCT_EMOTESPACE..spell; 
				elseif (msgtype == "gain") then
					msgtodisplay = mob..SASCT_EMOTESPACE.. SASCT_GAINS ..SASCT_EMOTESPACE..spell;
				end
			end
		end
	end
...
function SpellAlertSCT_test()
	...
	if (SpellAlertSCTDB[playerName].Toggle ~= true) then
		SpellAlertSCT_print("SpellAlertSCT 已禁用.");
	else
		SpellAlertSCT_print("你可以在屏幕上显示出一条测试信息.");
	end
end
