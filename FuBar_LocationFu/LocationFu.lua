...
LocationFu.version = "2.0" .. string.sub("$Revision: 15887 $", 12, -3)
LocationFu.date = string.sub("$Date: 2006-11-03 23:57:58 -0500 (Fri, 03 Nov 2006) $", 8, 17)
LocationFu.hasIcon = true
LocationFu.clickableTooltip = true
...
function LocationFu:OnEnable()
	...
	if IsInInstance() and time() - startTime > 10 then -- instance
...
LocationFu:RegisterChatCommand(L["AceConsole-options"], options)
LocationFu.OnMenuRequest = options

function LocationFu:ZONE_CHANGED_NEW_AREA()
	SetMapToCurrentZone()
	self:UpdateData()
	if IsInInstance() then --fix this for BGs
		self:ScheduleRepeatingEvent("LocationFu", self.UpdateDisplay, 60, self)
	else
		self:ScheduleRepeatingEvent("LocationFu", self.UpdateDisplay, 0.1, self)
	end
...
local subZoneText, zoneText, pvpType, isArena
...
function LocationFu.MapFrame_OnUpdate(t)
	...
	if self:IsActive() then
		...
		if self:IsShowingMapOverlay() then
			...
			WorldMapFrameAreaLabel:SetTextColor(1, 1, 1)
			if zone ~= nil and (Tourist:IsZoneOrInstance(zone) or Tourist:DoesZoneHaveInstances(zone)) then
				...
				local low, high = Tourist:GetLevel(zone)
				if low ~= nil and high ~= nil and low > 0 and high > 0 then
				...
				if Tourist:DoesZoneHaveInstances(zone) then
					if lastZone ~= zone then
						...
						for instance in Tourist:IterateZoneInstances(zone) do
							...
							local r1, g1, b1 = Tourist:GetFactionColor(instance)
							local r2, g2, b2 = Tourist:GetLevelColor(instance)
							mapText = mapText .. "\n" .. string.format("|cff%02x%02x%02x%s|r", r1 * 255, g1 * 255, b1 * 255, instance)
							if low == high then
								mapText = mapText .. string.format(" |cff%02x%02x%02x[%d]|r", r2 * 255, g2 * 255, b2 * 255, high)
							else
								mapText = mapText .. string.format(" |cff%02x%02x%02x[%d-%d]|r", r2 * 255, g2 * 255, b2 * 255, low, high)
							end
							local groupSize = Tourist:GetInstanceGroupSize(instance)
							if groupSize > 0 then
								mapText = mapText .. " " .. string.format(L["%d-man"], groupSize)
							end
						end
