...
local MINOR_VERSION = "$Revision: 16014 $"
...
elseif locale == 'zhCN' then TAG_DUNGEON = "\229\156\176\228\184\139\229\159\142"
	...
	if numEntries > 0 then
		...
		for questid = 1, numEntries do
			...
			if isHeader then
				...
				if string.find(strQuestLogTitleText, '^%[') then _,_,strQuestLogTitleText = string.find(strQuestLogTitleText, "^%[?%d*.?]?%s?(.*)") end
	...
	if self.firstScanDone then
		...
		for name,id in pairs(quest_names) do
			if not oldquest_names[name] then
				...
				for l, goal in pairs(quests[id].leaderboard) do
					local oldid = oldquest_names[name]
					if oldquests[oldid].leaderboard[l].description == goal.description and oldquests[oldid].leaderboard[l].numGot ~= goal.numGot then
						self:TriggerEvent("Quixote_Leaderboard_Update", name, id, l, goal.description, oldquests[oldid].leaderboard[l].numGot, goal.numGot, goal.numNeeded, goal.type)
					end
				end
			end
		end
...
function Quixote:GetQuestByName(q)
	if string.find(q, '^%[') then _,_,q = string.find(q, "^%[?%d*.?]?%s?(.*)") end
