...
function GMail_AutoComplete()
	local text = GMailNameEditBox:GetText();
	_,_,text = strfind(text,"^%s*(.-)%s*$");
	...
	if ( num > 0 ) then
		for j=1, num, 1 do
			name = 	GetFriendInfo(j);
			if ( name and strlen(name) >= textlen ) then
				if ( string.sub(name,1,textlen) ==  string.sub(text,1,textlen) ) then
					GMailNameEditBox:SetText(name);
					GMailNameEditBox:HighlightText(textlen, -1);
					return;
				end;
			end;
		end;
