...
function UnitPopup_OnClick()
	if UnitPopupMenus[this.owner][this.value]=="BWPFOLLOW" then
		...
		if(not BWPFOLLOWPLAYER) then
			...
			BWP_SpecialDest = 1
			BWPFOLLOWPLAYER = 1
			DEFAULT_CHAT_FRAME:AddMessage("开始跟随"..BWPFOLLOW_Name)
			UnitPopupButtons["BWPFOLLOW"] = { text = "停止跟随", dist = 0 } 
		else
			BWPFOLLOWPLAYER = nil
			BWP_SetSpecial = false; 
			BWP_SpecialDest = 0;
			BWP_ClearDest()
			DEFAULT_CHAT_FRAME:AddMessage("不再跟随"..BWPFOLLOW_Name)
			UnitPopupButtons["BWPFOLLOW"] = { text = "设置为路点", dist = 0 } 
		end	
	end
...
function BWP_ClearDest() --Removes current destination
	...
	BWP_SetSpecial = false; 
	BWP_SpecialDest = 0;
	if(BWPFOLLOWPLAYER)then	BWPFOLLOWPLAYER = nil; UnitPopupButtons["BWPFOLLOW"] = { text = "设置为路点", dist = 0 }; end
	BWP_DisplayFrame:Hide();
...
function  AbandonQuest() 
	...
	if(title)then
		DEFAULT_CHAT_FRAME:AddMessage("放弃:"..title)
		clearquest(title)
	end
	Old_AbandonQuest()
...
function BWP_MiniDropMenu_Initialize()
	...
	if(tempitem_XBWP) then -- Make sure you have a valid object
	   if(tempitem_XBWP.X and tempitem_XBWP.Y) then -- Make sure X and Y coords Exist(means were in same zone)
		   info = {};
		   info.text = BWP_INFO_TEXT2..tempitem_XBWP.Name..BWP_INFO_TEXT3; --Option message "设置"..$name.."为路点"
		   ...
		   UIDropDownMenu_AddButton(info);
			...
			info = {};
			info.text = BWP_INFO_TEXT1..tempitem_XBWP.Name..BWP_INFO_TEXT3; --non click message "必须在同一地区 blah blah"
			info.isTitle = 1
			tempitem_XBWP = nil
			UIDropDownMenu_AddButton(info);
		end
