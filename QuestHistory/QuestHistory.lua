...
v10900-2
-- Added Chinese localization (courtesy Diablohu)
-- Split localization files for easier translation by other language-speakers.
...
function QuestHistory_SortComparison(index1, index2)
...
function QuestHistory_BuildSortedTable()
	local iNew = 1;
...
function QuestHistory_Detail_Update(questIndex)
	local spacerFrame = QuestHistoryDetailSpacerFrame;
	...
	if ( event == "PLAYER_LOGIN" ) then
		...
		if ( EarthFeature_AddButton ) then
			EarthFeature_AddButton(
				{
					id="QUESTHISTORY";
					name=QUESTHISTORY_FRAME_TITLE;
					subtext="";
					helptext=QUESTHISTORY_EARTH_HELPTEXT;
					icon="Interface\\Icons\\INV_Misc_Book_06";
					callback=QuestHistory_Toggle;
				}
			);
		end
...
function QuestHistory_GetQuestIndexString()
	if (DisplayedRealmName and DisplayedPlayerCharacterName) then
		return DisplayedRealmName,DisplayedPlayerCharacterName;
	else
		return nil,nil;
	end
end

function QuestHistory_GetSearchText()
	if (searchText=="") then
		searchText = nil;
	end
	return searchText;
...
function QuestHistory_GetCurrentSortOption()
	return currentSortType,currentSortOrder;
end

function QuestHistory_GetCurrentSortedID()
	return currentSortedID;
end

function QuestHistory_SetSortedTable(value)
	SortedTable = value;
	if (not SortedTable) then
		return;
	end
