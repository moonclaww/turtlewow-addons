DISTANCEWARNING_VERSION="DistanceWarning V 1.59 Beta 作者 雪夜之狼";
...
function DistanceWarning_OnLoad()
	...
	ChatFrame1:AddMessage(DISTANCEWARNING_VERSION.." Loaded");
...
function DistanceWarning_OnEvent(event,arg1)
	...
	if(event=="CHARACTER_POINTS_CHANGED" and DW_DETECT==1) then
		...
		elseif(DW_PLAYER_CLASS=="法师") then
			...
			local _,texture1,_,_,rank1,_,_,_=GetTalentInfo(2,5);
			local _,texture2,_,_,rank2,_,_,_=GetTalentInfo(3,9);
		...
		elseif(DW_PLAYER_CLASS=="德鲁伊") then
			...
			local _,texture,_,_,rank,_,_,_=GetTalentInfo(2,6); 
			if(not(texture) or rank==nil) then
				DW_NATUREREACH_RANK=0;
			else 
		...
		elseif(DW_PLAYER_CLASS=="牧师") then
			...
			local _,texture1,_,_,rank1,_,_,_=GetTalentInfo(2,10);
			local _,texture2,_,_,rank2,_,_,_=GetTalentInfo(3,10); 
			if(not(texture1) or rank1==nil) then
				DW_HOLYREACH_RANK=0;
			else
			...
			if(DW_SHADOWREACH_RANK~=rank0_2) then
				...
				if(DW_SHADOWREACH_RANK==1) then
					DW_SHADOW_INCREASE_RANGE=1;
				elseif(DW_SHADOWREACH_RANK==2) then
					DW_SHADOW_INCREASE_RANGE=3;
				else
					DW_SHADOW_INCREASE_RANGE=4;
				end
				if(rank0_2==1) then
					shadowrange0=1;
				...
				if(DW_ATTACK_ACTION_SLOT[20+shadowrange0]~=nil) then
					if(DW_ATTACK_ACTION_SLOT[20+DW_SHADOW_INCREASE_RANGE]==nil) then
						DW_ATTACK_NEEDSCAN[20+DW_SHADOW_INCREASE_RANGE]=true;
					end
					DW_ATTACK_ACTION_SLOT[20+shadowrange0]=nil;
					DW_ATTACK_NEEDSCAN[20+shadowrange0]=true;
					DW_NEEDSCAN=true;
				end 
				if(DW_ATTACK_ACTION_SLOT[30+rank0_2*2]~=nil) then
					if(DW_ATTACK_ACTION_SLOT[30+DW_SHADOWREACH_RANK*2]==nil) then
						DW_ATTACK_NEEDSCAN[30+DW_SHADOWREACH_RANK*2]=true;
					end
					DW_ATTACK_ACTION_SLOT[30+rank0_2*2]=nil;
					DW_ATTACK_NEEDSCAN[30+rank0_2*2]=true;
					DW_NEEDSCAN=true; 
				end 
			end
		end
...
function DistanceWarning_OnUpdate()
	...
	if(DW_NEEDSCAN) then
		local slotnum=1;
		while(slotnum<=120 and DW_NEEDSCAN) do
	...
	if(UnitName("target")) then
		local distance=DistanceWarning_GetDistance();
		if(distance~=DW_DISTANCE) then
			DW_DISTANCE=distance;
			DistanceWarning_SetStatusBar(distance);
		end
	end
end
...
function DistanceWarning_GetDistance()
	...
	if(CheckInteractDistance("target",1)) then
		InteractDistance=5.55;
		if(InteractDistance-DW_MELEE_MODIFYER<=1) then
			InteractDistance=1;
		else
			InteractDistance=InteractDistance-DW_MELEE_MODIFYER;
		end
	elseif(CheckInteractDistance("target",3)) then
		InteractDistance=10;
		if(InteractDistance-DW_RANGE_MODIFYER<=5) then
			InteractDistance=InteractDistance-DW_MELEE_MODIFYER;
		else
			InteractDistance=InteractDistance-DW_RANGE_MODIFYER;
		end
	elseif(CheckInteractDistance("target",2)) then
		InteractDistance=11.11;
		if(InteractDistance-DW_RANGE_MODIFYER<=5) then
			InteractDistance=InteractDistance-DW_MELEE_MODIFYER;
		else
			InteractDistance=InteractDistance-DW_RANGE_MODIFYER;
	...
	elseif(CheckInteractDistance("target",4)) then
		InteractDistance=30-DW_RANGE_MODIFYER;
	else
		InteractDistance=nil;
		--表示大于30码内定距离
	end
	...
	--综合分析以确定返回距离值
	if(SpellDistance) then
		if(InteractDistance and InteractDistance<SpellDistance) then
			return InteractDistance;
		else
			return SpellDistance;
		end
	else
		if(InteractDistance) then
			return InteractDistance;
		else
			return 200;
			--表示超出探测范围
...
function DistanceWarning_SpellDetector(slot)
	...
	elseif(DW_PLAYER_CLASS=="盗贼") then
		if(ico~=nil and lab==nil) then
			...
			if(DW_ATTACK_ACTION_SLOT[30+DW_THROWING_RANK*3]==nil and (ActionName=="投掷")) then
				yard=30+DW_THROWING_RANK*3;
				DW_ATTACK_ACTION_SLOT[yard]=slot;
				DW_ATTACK_NEEDSCAN[yard]=false;
	...
	elseif(DW_PLAYER_CLASS=="法师") then
		...
		if(ico~=nil and lab==nil) then
			...
			if(DW_ATTACK_ACTION_SLOT[30+DW_FRAMETHROWING_RANK*3]==nil and (ActionName=="灼烧" 
			or (ActionName=="火球术" and ActionRank=="等级 1"))) then
	...
	elseif(DW_PLAYER_CLASS=="萨满祭司") then
		if(ico~=nil and lab==nil) then 
			...
			if(DW_ATTACK_ACTION_SLOT[30]==nil and (ActionName=="闪电链" or ActionName=="闪电箭" 
			or ActionName=="净化术")) then 
				yard=30;
				DW_ATTACK_ACTION_SLOT[yard]=slot;
				DW_ATTACK_NEEDSCAN[yard]=false;
	...
	elseif(DW_PLAYER_CLASS=="牧师") then
		if(ico~=nil and lab==nil) then
			...
			if(DW_ATTACK_ACTION_SLOT[20+DW_SHADOW_INCREASE_RANGE]==nil and ActionName=="精神鞭笞") then
				yard=20+DW_SHADOW_INCREASE_RANGE;
				DW_ATTACK_ACTION_SLOT[yard]=slot;
				DW_ATTACK_NEEDSCAN[yard]=false;
			
			end
			if(DW_ATTACK_ACTION_SLOT[30+DW_SHADOWREACH_RANK*2]==nil and (ActionName=="暗言术：痛" 
			or ActionName=="心灵震爆" or ActionName=="吸血鬼的拥抱" or ActionName=="噬灵瘟疫")) then
				yard=30+DW_SHADOWREACH_RANK*2;
				DW_ATTACK_ACTION_SLOT[yard]=slot;
				DW_ATTACK_NEEDSCAN[yard]=false;
...
function DistanceWarning_CheckScanNeed()
	...
	elseif(DW_PLAYER_CLASS=="牧师") then
		...
		or DW_ATTACK_NEEDSCAN[20+DW_SHADOW_INCREASE_RANGE] 
		or DW_ATTACK_NEEDSCAN[30+DW_SHADOWREACH_RANK*2] or DW_ATTACK_NEEDSCAN[30] 
		or DW_ASSIST_NEEDSCAN[15] or DW_ASSIST_NEEDSCAN[30] or DW_ASSIST_NEEDSCAN[40]);
	end
	return needscan;
end
...
--根据不同职业设置进度条
function DistanceWarning_SetStatusBar(dis)
	local text;
	if(DW_PLAYER_CLASS=="猎人") then
		if(dis<=5) then
		...
		elseif(dis<=15+DW_TOWNWATCH_RANK*2) then
			DistanceWarningFrameStatusBar:SetStatusBarColor(1,1,0);
		elseif(dis<=30) then
			DistanceWarningFrameStatusBar:SetStatusBarColor(1,0.5,0);
		elseif(dis<=35+DW_TOWNWATCH_RANK*2) then
	...
	DistanceWarningFrameStatusBar:SetValue(dis);
