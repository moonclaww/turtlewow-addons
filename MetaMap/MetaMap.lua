...
function MetaMap_ZSMOnShow()
	if(GetLocale() == "deDE") then
			MetaMap_OptionsInfo:SetText("发现德文客户端");
	elseif(GetLocale() == "frFR") then
			MetaMap_OptionsInfo:SetText("发现法文客户端");
	else
		MetaMap_LoadZSMButton:Disable();
		MetaMap_OptionsInfo:SetText("发现简体中文客户端 - 不需要地区转换");
	end
end
...
function MetaMap_LoadZSM()
	...
	if(IsAddOnLoaded("MetaMapZSM")) then
		...
		MetaMap_LoadZSMButton:Disable();
		MetaMap_OptionsInfo:SetText("MetaMapZSM "..METAMAP_NOMODULE);
	end
end
...
function MetaMap_LoadBKP()
	...
	if(IsAddOnLoaded("MetaMapBKP")) then
		...
		if(MetaMap_GeneralDialog:IsVisible()) then
			MetaMap_OptionsInfo:SetText("MetaMapWKB "..METAMAP_NOMODULE);
		else
			MetaMap_StatusPrint("MetaMapWKB "..METAMAP_NOMODULE, true);
		end
	end
end
...
function MetaMap_LoadBLT(lootID, Name)
	...
	if(IsAddOnLoaded("MetaMapBLT")) then
		...
		MetaMap_StatusPrint("MetaMapBLT "..METAMAP_NOMODULE, true);
	end
end
...
function MetaMap_LoadBWP(id, mode)
	...
	if(IsAddOnLoaded("MetaMapBWP")) then
		...
		MetaMap_StatusPrint("MetaMapBWP "..METAMAP_NOMODULE, true);
	end
end
...
function MetaMap_LoadQST(mode)
	...
	if(IsAddOnLoaded("MetaMapQST")) then
		...
		MetaMap_StatusPrint("MetaMapQST "..METAMAP_NOMODULE, true);
	end
end
...
function MetaMap_LoadFWM(mode)
	...
	if(IsAddOnLoaded("MetaMapFWM")) then
		...
		if(MetaMap_ModulesDialog:IsVisible()) then
			...
			MetaMap_StatusPrint("MetaMapFWM "..METAMAP_NOMODULE, true);
		end
	end
end
...
function MiniMapCoords_OnUpdate()
	if (MetaMapOptions.MetaMapMiniCoords and Minimap:IsVisible()) then
		...
		local px, py = GetPlayerMapPosition("player");
		if(zone == 0) then
			MetaMapMiniCoords:SetText("死亡区域");
...
function MetaMapShadeSet_Toggle()
	if(MetaMapOptions.ShadeSet == 4) then
		MetaMapOptions.ShadeSet = 1;
	else
		MetaMapOptions.ShadeSet = MetaMapOptions.ShadeSet +1;
	end
	MetaMapOptions_Init();
end
...
function MetaMap_ShowLocation(zoneName, noteName, noteID)
	...
	if(not noteID) then
		for index, value in currentZone, 1 do
			if(currentZone[index].name == noteName) then
				noteID = index;
				break;
			end
		end
...
function MetaMap_ZoneHeader()
	...
	continent = "|cfff0B300当前大陆: |cffffffff"..continent;
	zone = "   |cfff0B300当前地区: |cffffffff"..mapName.." ("..zone..")";
	realContinent = "|cfff0B300所在大陆: |cffffffff"..realContinent;
	realZone = "   |cfff0B300所在地区: |cffffffff"..GetRealZoneText().." ("..realZone..")";
...
function MetaMap_ZoneTableSize(zoneName)
	local continent, zone = MetaMap_NameToZoneID(zoneName);
	local zoneTable = MetaMapNotes_Data[continent][zone];
	local i = 0;
	for index in zoneTable do
		i = i + 1;
	end
	return i;
end
...
function MetaMap_DeleteNotesListInit()
	...
	UIDropDownMenu_SetText("[点击选择创建者]", MetaMap_DeleteNotesList)
end
...
function MetaMap_DeleteNotes(creator, name, zoneName)
	...
	if(zoneName ~= nil) then
		if(MetaMap_NotesDialog:IsVisible()) then
			MetaMap_OptionsInfo:SetText(format(METAMAPNOTES_DELETED_BY_ZONE, zoneName, creator), true);
		else
...
function MetaMapNotes_DeleteNote(id, continent, zone)
	...
	if(continent == nil or zone == nil) then
		continent, zone, currentZone = MetaMap_GetCurrentMapInfo();
	else
		currentZone = MetaMapNotes_Data[continent][zone];
	end
	TempData[continent] = {};
...
function MetaMapPOI_OnEvent(mode)
	...
	if(noteAdded1 and noteAdded2) then
		MetaMap_StatusPrint("MetaMapPOI 更新地图标记在 "..GetRealZoneText(), true);
	else
		if(noteAdded2) then
			MetaMap_StatusPrint("MetaMapPOI 更新地图标记在 "..GetRealZoneText(), true);
		end
		if(noteAdded1) then
			MetaMap_StatusPrint("MetaMapPOI 设置警戒标记在 "..name, true);
		end
	end
end
...
function MetaMap_InfoLine(button)
	...
	if(MetaMapFrame:IsVisible()) then
		...
		MetaMap_InfoLineFrameText:SetText("|cffffffff更多的信息也许会在稍后加入...|r");
	end
...
function MetaMap_InfoLineOnEnter()
	...
	if(MetaMapFrame:IsVisible()) then
		...
		WorldMapTooltip:AddDoubleLine("副本进度:", "ID:");
		if(GetNumSavedInstances() > 0) then
			...
			WorldMapTooltip:AddLine("|cffffffff没有副本进度");
		end
	else
		local continent, zone, currentZone, mapName = MetaMap_GetCurrentMapInfo();
...
function MetaMap_SetNUNtooltip()
	if(IsAddOnLoaded("NotesUNeed")) then
		...
		NuN_MapTooltip:SetScale(NuNSettings[pKey].mScale/1.25);
		NuNPopup:SetScale(0.001);
	end
end