...
function QST_UpdateFormat()
	for index, qLog in QST_QuestLog do
		if(not qLog.qLogged) then
			qLog.qLogged = time() - MetaMap_round(qLog.qLogtime);
		end
		qLog.qLogtime = nil;
	end
	for index, qBase in QST_QuestBase do
...
function QST_VerifyLog()
	for logIndex, qLog in QST_QuestLog do
		if(qLog.qPlayer[QST_ActivePlayer]) then
			...
			for qIndex=1, GetNumQuestLogEntries(), 1 do
				...
				if(not isHeader and qLog.qTitle == QuestTitle) then
					orphan = false;
				end
			end
			if(orphan) then
				if(QST_Options.SaveHistory) then
					...
					for baseIndex, qQuest in QST_QuestBase do
...
function QST_QuickMenuInit()
	for index, value in QST_QuickMenu do
		local spacer = nil;
	...
	elseif(this.value == QST_MENU_GIVER) then
		if(qData.qNPC[1]) then
			if(QST_SetMapNote(qData, 1)) then
				...
				MetaMap_StatusPrint("MapQuery", true)
					local noteID = MetaMap_ZoneTableSize(qData.qNPC[1].qZone);
	...
	elseif(this.value == QST_MENU_COMP) then
		if(qData.qNPC[2]) then
			if(QST_SetMapNote(qData, 2)) then
				if(QST_Options.MapQuery) then
					local noteID = MetaMap_ZoneTableSize(qData.qNPC[2].qZone);
...
function QSTButton_OnEnter()
	...
	elseif(IsShiftKeyDown()) then
		WorldMapTooltip:SetText(qData.qTitle, 1, 1, 1, 1);
		if(qData.qNPC[1]) then
			WorldMapTooltip:AddDoubleLine("开始:", QST_FormatTime(qData.qLogged), NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, 1);
		...
		if(qData.qNPC[2]) then
			WorldMapTooltip:AddDoubleLine("结束:", QST_FormatTime(qData.qEnded), NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, 1);
...
function QST_HintTooltip()
	...
	WorldMapTooltip:AddDoubleLine("Shift-按住", QST_TTHINT_T0, 1, 1, 1, 1);
	WorldMapTooltip:AddDoubleLine("Ctrl-按住", QST_TTHINT_T1, 1, 1, 1, 1);
	WorldMapTooltip:AddDoubleLine("左键", QST_TTHINT_T2, 1, 1, 1, 1);
	WorldMapTooltip:AddDoubleLine("右键", QST_TTHINT_T3, 1, 1, 1, 1);
	WorldMapTooltip:AddLine(QST_TTHINT_H2, 0, 1, 0, 1);
	WorldMapTooltip:AddDoubleLine("Shift-左键", QST_TTHINT_T4, 1, 1, 1, 1);
	WorldMapTooltip:AddDoubleLine("Ctrl-左键", QST_TTHINT_T5, 1, 1, 1, 1);
	WorldMapTooltip:Show();
end
