...
function sctc_OnEvent(event, arg1)
	if (event == "VARIABLES_LOADED") then
		...
		if (not sctcOptions["version"]) then
			...
			sctcOptions["format"] = "[%s 可以再次施放]";
			sctcOptions["wformat"] = "[%s 将在10秒后可用]";
			sctcOptions["color"] = {};
...
function sctc_OnUpdate()
	for key, val in sctcVars.spells do
		...
		if (sctcVars["cooldowns"][name] and not sctcOptions["disabled"][name]) then
			...
			local found = false;
			local group;
			for key in SCTC_GROUPS do	sctc_debugp("在队伍中检查匹配: "..key);
				for val in SCTC_GROUPS[key] do
					if (name == SCTC_GROUPS[key][val]) then	sctc_debugp("发现匹配: "..SCTC_GROUPS[key][val]);
						found = true;
						group = key;
					end
				end
			...
			if (sctcOptions["warning"] ~= nil) then
				...
				if (timeleft == sctcOptions["warning"] and sctcVars["warnings"][name]) then sctc_debugp("剩余时间: "..timeleft);
					if (found) then
						SCT_Display(string.format(sctcOptions["wformat"], group), sctcOptions["color"], sctcOptions["crit"]);
						for key, val in SCTC_GROUPS[group] do	sctc_debugp("正在清除提醒: "..val);
			...
			if (duration == 0) then	sctc_debugp(name.."持续0秒");
				if (found) then
					SCT_Display(string.format(sctcOptions["format"], group), sctcOptions["color"], sctcOptions["crit"]);
					for key, val in SCTC_GROUPS[group] do	sctc_debugp("正在清除冷却时间: "..val);
		...
		if (duration > minDuration) then
			if (startTime ~= sctcVars["cooldowns"][name]) then
				if (not sctcOptions["disabled"][name]) then
					sctcVars["cooldowns"][name] = startTime; sctc_debugp(name.." 添加到冷却时间中");
					if (sctcOptions["warning"] ~= nil and duration > sctcOptions["warning"]) then
						sctcVars["warnings"][name] = startTime;	sctc_debugp(name.." 添加到提醒中");
					end
					sctcVars.numCds = sctcVars.numCds + 1;
				end
			end
