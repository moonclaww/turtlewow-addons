...
--
--	Modified by iWOW (http://www.365wave.com/wow)
--
...
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_AXE][0] = AXE;
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_AXE][1] = LOOTLINK_2H_AXE;
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_BOW] = BOW;
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_DAGGER] = DAGGER;
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_MACE] = { }
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_MACE][0] = LOOTLINK_1H_MACE;
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_MACE][1] = LOOTLINK_2H_MACE;
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_STAFF] = STAFF;
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_SWORD] = { };
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_SWORD][0] = LOOTLINK_1H_SWORD;
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_SWORD][1] = LOOTLINK_2H_SWORD;
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_GUN] = GUN;
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_WAND] = WAND;
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_THROWN] = THROWN;
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_POLEARM] = POLEARM;		--@todo Telo: unconfirmed
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_FIST_WEAPON] = FIST_WEAPON;
lTypeAndSubtypeToSkill[TYPE_WEAPON][SUBTYPE_WEAPON_CROSSBOW] = CROSSBOW;
...
local function LootLink_GetHyperlink(name)
	...
	if( itemLink and itemLink.i and LootLink_CheckItemServer(itemLink, lServerIndex) ) then
		...
		local item = string.gsub(itemLink.i, "(%d+):(%d+):(%d+):(%d+)", "%1:0:%3:0");
		return "item:"..item;
	end
	return nil;
end
	...
	for index = 1, 30, 1 do
		...
		while( loop ) do
			...
			elseif( state == STATE_BOUND ) then
				iStart, iEnd, val1 = string.find(left, LOOTLINK_FILTER_BIND);
				if( val1 ) then
					if( val1 == LOOTLINK_FILTER_EQUIP ) then
						value.d = value.d.."bi"..BINDS_EQUIP.."·";
					elseif( val1 == LOOTLINK_FILTER_PICKUP ) then
						value.d = value.d.."bi"..BINDS_PICKUP.."·";
					elseif( val1 == LOOTLINK_FILTER_USE ) then
			...
			elseif( state == STATE_UNIQUE ) then
				if( string.find(left, ITEM_UNIQUE)) then
					value.d = value.d.."un1·";
					loop = nil;
				end
			...
			elseif( state == STATE_DAMAGE ) then
				iStart, iEnd, val1, val2 = string.find(left, LOOTLINK_FILTER_DAMAGE);
				if( val1 and val2 ) then
					value.d = value.d.."mi"..val1.."·ma"..val2.."·";
				end

				iStart, iEnd, val1 = string.find(right, LOOTLINK_FILTER_SPEED);
				if( val1 ) then
			...
			elseif( state == STATE_DPS ) then
				iStart, iEnd, val1 = string.find(left, LOOTLINK_FILTER_DPS);
				if( val1 ) then
			...
			elseif( state == STATE_ARMOR ) then
				iStart, iEnd, val1 = string.find(left, LOOTLINK_FILTER_ARMOR);
				if( val1 ) then
			...
			elseif( state == STATE_BLOCK ) then
				iStart, iEnd, val1 = string.find(left, LOOTLINK_FILTER_BLOCK);
				if( val1 ) then
			...
			elseif( state == STATE_CONTAINER ) then
				iStart, iEnd, val1 = string.find(left, LOOTLINK_FILTER_CONTAINER);
				if( val1 ) then
			...
			elseif( state == STATE_REQUIRES ) then
				iStart, iEnd, val1 = string.find(left, LOOTLINK_FILTER_REQUIRE);
				if( val1 ) then
					foundRequires = 1;
					loop = nil;
					iStart, iEnd, val2 = string.find(val1, LOOTLINK_FILTER_LEVEL);
					if( val2 ) then
						value.d = value.d.."le"..val2.."·";
					else
						iStart, iEnd, val2, val3 = string.find(val1, LOOTLINK_FILTER_SKILL);
...
local function LootLink_SetHyperlink(tooltip, name, link)
	...
	if( not GetItemInfo(link) ) then
		...
		if( value ) then
			...
			if( unique ) then
				tooltip:AddLine(ITEM_UNIQUE, 1, 1, 1);
				lines = lines + 1;
			end
			...
			if( _type == TYPE_ARMOR ) then
				...
				if( armor ) then
					tooltip:AddLine(format(ARMOR_TEMPLATE,tonumber(armor)), 1, 1, 1);
					lines = lines + 1;
				end
			elseif( _type == TYPE_WEAPON ) then
				...
				if( min and max ) then
					tooltip:AddLine(format(DAMAGE_TEMPLATE,tonumber(min),tonumber(max)), 1, 1, 1);
					lines = lines + 1;
					if( speed ) then
						local line = getglobal(tooltip:GetName().."TextRight"..lines);
						line:SetText(SPEED.." "..speed);
						line:SetTextColor(1, 1, 1);
						line:Show();
						extraSkip = extraSkip + 1;
					end
				...
				if( dps ) then
					tooltip:AddLine(format(DPS_TEMPLATE,tonumber(dps)), 1, 1, 1);
					lines = lines + 1;
				end
			elseif( _type == TYPE_SHIELD ) then
				...
				if( block ) then
					tooltip:AddLine(format(SHIELD_BLOCK_TEMPLATE,tonumber(block)), 1, 1, 1);
					lines = lines + 1;
				end
					...
					for i, v in RecipeSubtypes do
						if( v == subtype ) then
							...
							if( not rank or rank < skill ) then
								tooltip:AddLine(format(ITEM_MIN_SKILL,i,tonumber(skill)), 1, 0, 0);
							else
								tooltip:AddLine(format(ITEM_MIN_SKILL,i,tonumber(skill)), 1, 1, 1);
							end
							lines = lines + 1;
							break;
						end
			...
			elseif( _type == TYPE_CONTAINER ) then
				...
				if( slots ) then
					tooltip:AddLine(format(LOOTLINK_TOOLTIP_CONTAINER,tonumber(slots)), 1, 1, 1);
					lines = lines + 1;
				end
			end
			...
			if( value.t ) then
				...
				for piece in string.gfind(value.t, "(.-)") do
					if( lines < 29 ) then
						if( skip == 0 ) then
							if( string.find(piece, LOOTLINK_FILTER_REQUIRELEVEL) ) then
								if( level and tonumber(level) > UnitLevel("player") ) then
									tooltip:AddLine(piece, 1, 0, 0, 1, 1);
								else
									tooltip:AddLine(piece, 1, 1, 1, 1, 1);
								end
							elseif( string.find(piece, LOOTLINK_FILTER_REQUIRESPILIT) ) then
			...
			if( lines < 30 ) then
				tooltip:AddLine(LOOTLINK_TOOLTIP_INFO);
			end
		...
		EarthFeature_AddButton(
			{
				id="LOOTLINK";
				name=LOOTLINK_TITLE;
				subtext=LOOTLINK_SHORTDESC;
				helptext=LOOTLINK_DESC;
				icon="Interface\\Icons\\INV_Misc_Lantern_01";
				callback=ToggleLootLink;
			}
		);
	end
end
		...
		for color, item, name in string.gfind(text, "|c(%x+)|Hitem:(%d+:%d+:%d+:%d+)|h%[(.-)%]|h|r") do
			if( color and item and name and name ~= "" ) then
				if( not ItemLinks[name] ) then
					...
					ItemLinks[name].i = string.gsub(item, "^(%d+):(%d+):(%d+):(%d+)$", "%1:0:%3:0");
					lItemLinksSizeTotal = lItemLinksSizeTotal + 1;
				...
				elseif( trusted ) then
					...
					local link = string.gsub(item, "^(%d+):(%d+):(%d+):(%d+)$", "%1:0:%3:0");
