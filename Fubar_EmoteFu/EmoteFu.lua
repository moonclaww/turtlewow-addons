...
Change Log:
v1.12-2.0-0.1 (TOC 11200)
-- Re-write of the EmoteFu code for FuBar v2.0
-- New numbering scheme, which is v(TOC)-(FuBar vers)-(iteration)
v0.1.5 (TOC 11200)
...
local L = AceLibrary("AceLocale-2.0"):new("FuBar_EmoteFu")

EmoteFu = AceLibrary("AceAddon-2.0"):new("FuBarPlugin-2.0", "AceDB-2.0", "AceConsole-2.0", "AceEvent-2.0")
...
EmoteFu.version = "1.12-2.0-0.1"
EmoteFu.hasIcon = "Interface\\Icons\\Spell_Shadow_Charm"
EmoteFu.hasText = "EmoteFu"

function EmoteFu:IsShowingSlashCommands()
	return self.db.profile.showSlashCommand
end
...
local options = {
	...
	args = {
		slash = {
			...
			name = "显示斜杠命令",
			desc = "显示/隐藏表情对应的斜杠命令",
			get = "IsShowingSlashCommands",
			set = "ToggleShowingSlashCommands"
		}
	}
}

EmoteFu:RegisterChatCommand({ "/emotefu", "/emofu" }, options)
...
function EmoteFu:OnMenuRequest(level, value)
	if level == 1 then
